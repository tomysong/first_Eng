...
v2의 구체적인 친구 대화, 게임 요소, 원어민-내 발음 비교 고도화 범위는
`V2_UPDATE_PLAN.md`에 따로 정리한다.
...
7. ~~**콘텐츠 순환 노출**~~ ✅ 커리큘럼 반복 수정 완료 (2026-06-22) — 레벨별 일반 미션을 24개로 확장하고 Day 7/14/21/28만 보스 미션으로 유지해 4주 계획이 매주 다르게 보이도록 수정. 대화문은 현재 레벨별 14세트라 후반부 확장 여지는 남아 있다.
...
### 완료된 확장 (2026-06-11)
- **로그인 없는 다중 프로필**: 프로필별 localStorage 네임스페이스(`kidEnglish.<id>.*`), 첫 프로필 "서율"(영어 이름 `Seo-Yul`)은 기존 키를 이어받아 데이터 마이그레이션 불필요. AI 키·목소리는 기기 공통. 기기 간 동기화가 필요할 때만 서버 로그인 도입.
